...
// Secure API Key Manager with enhanced security features
class ApiKeyManager {
  private static instance: ApiKeyManager;
  private apiKey: string = process.env["KHULNASOFT_API_KEY"] || "";
  private isSet: boolean = Boolean(process.env["KHULNASOFT_API_KEY"]);
  private lastUsed: number = 0;
  private usageCount: number = 0;
  private readonly RATE_LIMIT = 100; // Max API calls per minute
  ...
  public setApiKey(apiKey: string): void {
    ...
    this.apiKey = apiKey;
    this.isSet = true;
    this.usageCount = 0; // Reset usage counter on new key
    this.lastUsed = 0;
    this.logKeyUsage('api_key_set');
  }

  /**
   * Rotate the current API key to a new one
   * @param newApiKey The new API key to use
   * @returns The previous API key (useful for rollback scenarios)
   */
  public rotateApiKey(newApiKey: string): string | null {
    if (typeof newApiKey !== 'string' || !newApiKey.trim()) {
      throw new Error('New API key must be a non-empty string');
    }
    
    const oldKey = this.apiKey;
    this.apiKey = newApiKey;
    this.isSet = true;
    this.usageCount = 0; // Reset usage counter on key rotation
    this.lastUsed = 0;
    
    this.logKeyUsage('api_key_rotated');
    return oldKey || null;
  }

  /**
   * Check if the current key is being rate limited
  ...
  public clearApiKey(): void {
    this.logKeyUsage('api_key_cleared');
    this.apiKey = '';
    this.isSet = false;
    this.usageCount = 0;
    this.lastUsed = 0;
  ...
  /**
   * Log key usage events without exposing the actual key
   * @param eventType Type of event to log
   */
  private logKeyUsage(eventType: string): void {
  ...
  /**
   * Generate a fingerprint of the key for logging (not reversible)
   * @returns A hashed fingerprint of the key
   */
...
/**
 * @deprecated Use apiKeyManager.setApiKey() instead
 */
